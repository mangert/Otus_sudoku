...
| `app/app.cpp` | `SudokuApp` | Интерактивный выбор размера, загрузка доски из TXT/JSON или ввод с клавиатуры, решение и сохранение результата. |
...
| `tests/app/test_sudoku_json_io.cpp` | `test_sudoku_json_io` | Тесты чтения, проверки и сохранения досок в JSON. |
...
| `app/` | Консольные меню, ввод с клавиатуры, чтение и запись TXT/JSON, отображение результата и ошибок. |
...
- Пустая клетка обозначается `0`; TXT содержит `N` строк по `N` чисел, а JSON — поля `size` и двумерный массив `board`.
